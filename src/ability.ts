...
import type { AppState, DiagnosticAxis } from "./core/types.js";

export type DisplayAbilityId = "STR" | "CON" | "SIZ" | "DEX" | "APP" | "INT" | "POW" | "EDU" | "SAN" | "LUCK";
export type DiceFamily = "3D6" | "2D6+6" | "3D6+3" | "3D6x5";

export interface AbilityMeta {
  id: DisplayAbilityId;
  sourceAxis: DiagnosticAxis;
  label: string;
  shortLabel: string;
  diceFamily: DiceFamily;
  betaScale: number;
  highTag: string;
  lowTag: string;
...
export interface AbilityResult extends AbilityMeta {
  theta: number;
  percentile: number;
  value: number;
...
const ABILITY_META: readonly AbilityMeta[] = [
  {
...
const THREE_D6_COUNTS = [1, 3, 6, 10, 15, 21, 25, 27, 27, 25, 21, 15, 10, 6, 3, 1] as const;
const TWO_D6_COUNTS = [1, 2, 3, 4, 5, 6, 5, 4, 3, 2, 1] as const;
...
function diceTable(family: DiceFamily): Array<{ value: number; count: number; total: number }> {
  if (family === "2D6+6") {
...
function percentileToDice(percentile: number, family: DiceFamily): { value: number; probability: number } {
...
export function getAbilityResults(state: AppState): AbilityResult[] {
...
export function getDistinctiveAbilities(results: readonly AbilityResult[], count = 2): AbilityResult[] {
  return [...results].sort((left, right) => right.distinctiveness - left.distinctiveness).slice(0, count);
}

export function buildProfileTitle(results: readonly AbilityResult[]): string {
  const [first, second] = getDistinctiveAbilities(results, 2);
  if (!first || !second) return "選択の痕跡が残る探索者";
...
export function buildProfileSummary(results: readonly AbilityResult[]): string {
  const [first, second] = getDistinctiveAbilities(results, 2);
  if (!first || !second) return "あなたの選択から、複数の行動傾向が観測されました。";
...
export function abilityById(results: readonly AbilityResult[], id: DisplayAbilityId): AbilityResult {
  const found = results.find((item) => item.id === id);
  if (!found) throw new Error(`Ability ${id} not found`);
  return found;
}

export function allDiagnosticAxes(): readonly DiagnosticAxis[] {
