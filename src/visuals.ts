import type { EndingDefinition } from "./core/types.js";
...
export function drawRadarChart(canvas: HTMLCanvasElement, abilities: readonly AbilityResult[]): void {
  const size = 760;
  ...
  const angleAt = (index: number): number => -Math.PI / 2 + (Math.PI * 2 * index) / count;
...
export async function createResultCardBlob(input: ShareCardInput): Promise<Blob> {
  const canvas = document.createElement("canvas");
  canvas.width = 1080;
  canvas.height = 1350;
  const context = canvas.getContext("2d");
  if (!context) throw new Error("Canvasを使用できません");
