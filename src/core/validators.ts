...
import type { DiagnosticAxis, StateEffect, ValidationIssue } from "./types.js";

const ALLOWED_WEIGHTS = new Set([-1, -0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 1]);

function validateEffect(effect: StateEffect, issues: ValidationIssue[], slotId: string, choiceId: string): void {
  if (effect.op !== "add") return;
...
export function validateStaticModel(flowFileContent?: string): ValidationIssue[] {
  ...
  const primaryCounts = Object.fromEntries(DIAGNOSTIC_AXES.map((axis) => [axis, 0])) as Record<DiagnosticAxis, number>;
  ...
  for (const scene of SCENES) {
    ...
    for (const item of scene.choices) {
      ...
      for (const [axis, weight] of Object.entries(item.diagnosticWeights) as [DiagnosticAxis, number][]) {
      ...
      for (const effect of item.effects) validateEffect(effect, issues, scene.slotId, item.id);
      for (const effect of item.diceEffects?.success ?? []) validateEffect(effect, issues, scene.slotId, item.id);
      for (const effect of item.diceEffects?.failure ?? []) validateEffect(effect, issues, scene.slotId, item.id);
      for (const option of item.followUp?.options ?? []) {
        for (const effect of option.effects) validateEffect(effect, issues, scene.slotId, item.id);
      }
    }
  ...
  const baseSceneCounts = Object.fromEntries(DIAGNOSTIC_AXES.map((axis) => [axis, 0])) as Record<DiagnosticAxis, number>;
  ...
  for (const axis of ["STR", "CON", "SIZ", "DEX", "APP", "INT", "POW", "EDU"] as DiagnosticAxis[]) {
...
export function validateRuntimeStateInvariants(state: import("./types.js").AppState): string[] {
