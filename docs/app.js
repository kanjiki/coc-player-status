...
import { buildProfileSummary, buildProfileTitle, getAbilityResults, getDistinctiveAbilities } from "./ability.js";
...
import { applyChoice, cloneState, fixationClueCount, networkClueCount, resolveScene, sealClueCount, stopClueCount } from "./core/engine.js";
...
function remainingTimeLabel(state) {
    if (state.story.timeUnits >= 8)
        return "まだ余裕がある";
    if (state.story.timeUnits >= 4)
        return "少なくなってきた";
    if (state.story.timeUnits >= 1)
        return "ほとんど残っていない";
...
function choiceCardMarkup(choice, index) {
    return `
...
function sliderBand(config, value) {
    return config.bands.find((band) => value <= band.max) ?? config.bands.at(-1);
}
function renderSliderResponse(scene, config) {
...
function mergeWeightedEvidence(target, weights, multiplier) {
    for (const [axis, weight] of Object.entries(weights)) {
        const typedAxis = axis;
        target[typedAxis] = (target[typedAxis] ?? 0) + weight * multiplier;
    }
}
function allocationAxisEvidence(scene, config, values) {
    const evidence = {};
...
function rankingAxisEvidence(scene, order) {
    const rankWeights = [1, 0.6, 0.3, 0];
    const denominator = rankWeights.slice(0, order.length).reduce((sum, value) => sum + value, 0) || 1;
    const evidence = {};
...
function renderQuadrantResponse(scene, config) {
...
function renderAllocationResponse(scene, config) {
    return `
...
function renderRankingResponse(scene, config, state) {
...
function renderResponseInterface(scene, state) {
...
function surveyOptions(selected, values) {
...
function renderResult() {
    ...
    requestAnimationFrame(() => {
        ...
        if (canvas)
            drawRadarChart(canvas, abilities);
    });
...
function activeStructuredScene(kind) {
    if (!session || session.phase !== "scene")
        return null;
    const scene = resolveScene(session.state);
    const config = RESPONSE_CONFIGS[scene.slotId];
...
function updateSliderWorkbench(input) {
    ...
    const root = input.closest("[data-response-kind='slider']");
    if (!root)
        return;
    const valueElement = root.querySelector("[data-response-value]");
    const preview = root.querySelector("[data-response-preview]");
    const submit = root.querySelector("[data-action='submit-slider']");
    if (valueElement)
        valueElement.textContent = String(value);
    if (preview)
        preview.innerHTML = `<span class="response-value">${value}</span><div><strong>${escapeHtml(band.label)}</strong><p>${escapeHtml(band.explanation)}</p></div>`;
    if (submit)
        submit.disabled = false;
}
function submitSliderResponse() {
    const active = activeStructuredScene("slider");
    const input = document.querySelector("[data-slider-control]");
    if (!active || !input || input.dataset.touched !== "true")
        return;
    const value = Number(input.value);
    const band = sliderBand(active.config, value);
    requestChoice(band.choiceId, {
        kind: "slider",
        value,
...
function updateQuadrantWorkbench(input) {
    const active = activeStructuredScene("quadrant");
    if (!active)
        return;
    input.dataset.touched = "true";
    const root = input.closest("[data-response-kind='quadrant']");
    if (!root)
        return;
    const xInput = root.querySelector("[data-quadrant-axis='x']");
    const yInput = root.querySelector("[data-quadrant-axis='y']");
    if (!xInput || !yInput)
        return;
    ...
    const preview = root.querySelector("[data-response-preview]");
    const submit = root.querySelector("[data-action='submit-quadrant']");
    if (preview) {
    ...
    if (submit)
        submit.disabled = !(xInput.dataset.touched === "true" && yInput.dataset.touched === "true");
}
function submitQuadrantResponse() {
    const active = activeStructuredScene("quadrant");
    const root = document.querySelector("[data-response-kind='quadrant']");
    const xInput = root?.querySelector("[data-quadrant-axis='x']");
    const yInput = root?.querySelector("[data-quadrant-axis='y']");
    if (!active || !xInput || !yInput)
        return;
    if (xInput.dataset.touched !== "true" || yInput.dataset.touched !== "true")
        return;
    const x = Number(xInput.value);
    const y = Number(yInput.value);
    const key = quadrantKey(x, y);
...
function allocationValues(root, config) {
    const values = {};
    for (const item of config.items) {
        const element = root.querySelector(`[data-allocation-item='${item.id}']`);
        values[item.id] = Number(element?.dataset.count ?? 0);
    }
    return values;
}
function updateAllocationWorkbench(root, config) {
    ...
    const remainingElement = root.querySelector("[data-allocation-remaining]");
    if (remainingElement)
        remainingElement.textContent = String(remaining);
    for (const item of config.items) {
        const count = values[item.id] ?? 0;
        const countElement = root.querySelector(`[data-allocation-count='${item.id}']`);
        if (countElement)
            countElement.textContent = String(count);
        const minus = root.querySelector(`[data-item-id='${item.id}'][data-allocation-change='-1']`);
        const plus = root.querySelector(`[data-item-id='${item.id}'][data-allocation-change='1']`);
        if (minus)
    ...
    const preview = root.querySelector("[data-response-preview]");
    const submit = root.querySelector("[data-action='submit-allocation']");
    if (remaining > 0) {
        if (preview)
            preview.innerHTML = `<div><strong>残り${remaining}${escapeHtml(config.unitLabel)}</strong><p>すべての点を使うと決定できます。</p></div>`;
        if (submit)
...
function changeAllocation(button) {
    const active = activeStructuredScene("allocation");
    const root = button.closest("[data-response-kind='allocation']");
    const itemId = button.dataset.itemId;
    const delta = Number(button.dataset.allocationChange ?? 0);
    if (!active || !root || !itemId || !delta)
        return;
    const item = root.querySelector(`[data-allocation-item='${itemId}']`);
    if (!item)
        return;
    const values = allocationValues(root, active.config);
    const used = Object.values(values).reduce((sum, value) => sum + value, 0);
    const current = Number(item.dataset.count ?? 0);
    if (delta > 0 && used >= active.config.budget)
        return;
...
function submitAllocationResponse() {
    const active = activeStructuredScene("allocation");
    const root = document.querySelector("[data-response-kind='allocation']");
    if (!active || !root)
        return;
    const values = allocationValues(root, active.config);
    const used = Object.values(values).reduce((sum, value) => sum + value, 0);
    if (used !== active.config.budget)
        return;
    const max = Math.max(...Object.values(values));
...
function refreshRankingList(list) {
    const items = [...list.querySelectorAll("[data-choice-id]")];
    items.forEach((item, index) => {
        const position = item.querySelector(".ranking-position");
        if (position)
            position.textContent = String(index + 1);
        const up = item.querySelector("[data-rank-move='up']");
        const down = item.querySelector("[data-rank-move='down']");
        if (up)
            up.disabled = index === 0;
        if (down)
...
function moveRankingItem(button) {
    const list = button.closest("[data-ranking-list]");
    const item = button.closest("[data-choice-id]");
    if (!list || !item)
        return;
...
function submitRankingResponse() {
    ...
    requestChoice(top, { kind: "ranking", order, axisEvidence: rankingAxisEvidence(active.scene, order) }, `優先順位：${labels.map((label, index) => `${index + 1}. ${label}`).join(" ／ ")}`);
}
function requestChoice(choiceId, responseMetadata = { kind: "choice" }, responseSummary) {
    if (!session || session.phase !== "scene")
        return;
    const scene = resolveScene(session.state);
    ...
    if (choice.followUp) {
      ...
      <div class="dialog-actions"><button class="secondary-button" type="button" data-close-dialog>選び直す</button></div>`);
        dialog.querySelectorAll("[data-follow-up]").forEach((button) => {
            button.addEventListener("click", () => {
                const followUpId = button.dataset.followUp;
                dialog.close();
                if (followUpId)
                    commitChoice(choiceId, followUpId, responseMetadata, responseSummary);
            });
        });
    ...
    dialog.querySelector("[data-close-dialog]")?.addEventListener("click", () => dialog.close());
    dialog.querySelector("[data-confirm-choice]")?.addEventListener("click", () => {
        dialog.close();
...
function commitChoice(choiceId, followUpOptionId, responseMetadata = { kind: "choice" }, responseSummary) {
    ...
    if (result.followUpOption)
        pending.followUpLabel = result.followUpOption.label;
    if (result.diceRoll !== undefined)
        pending.diceRoll = result.diceRoll;
    if (result.diceSuccess !== undefined)
        pending.diceSuccess = result.diceSuccess;
    if (responseSummary)
        pending.responseSummary = responseSummary;
...
function confirmRestart() {
    ...
    dialog.querySelector("[data-close-dialog]")?.addEventListener("click", () => dialog.close());
    dialog.querySelector("[data-confirm-restart]")?.addEventListener("click", () => {
        dialog.close();
        clearSession();
        session = null;
        startNew();
...
function confirmClear() {
    ...
    dialog.querySelector("[data-close-dialog]")?.addEventListener("click", () => dialog.close());
    dialog.querySelector("[data-confirm-clear]")?.addEventListener("click", () => {
        dialog.close();
        clearAllLocalData();
        session = null;
        pageMode = "landing";
...
function resultText(ending, abilities) {
...
async function shareResult() {
    ...
    const shareData = { title: APP_CONFIG.appName, text, files: [file] };
...
app.addEventListener("click", (event) => {
    ...
    const allocationButton = target.closest("button[data-allocation-change]");
    if (allocationButton) {
        changeAllocation(allocationButton);
        return;
    }
    const rankButton = target.closest("button[data-rank-move]");
    if (rankButton) {
        moveRankingItem(rankButton);
        return;
    }
    const choiceButton = target.closest("button[data-choice-id]");
    ...
    switch (action) {
        ...
        case "clear":
            ...
            break;
        case "submit-slider":
            submitSliderResponse();
            break;
        case "submit-quadrant":
            submitQuadrantResponse();
            break;
        case "submit-allocation":
        ...
        case "submit-ranking":
            ...
            break;
        case "download-image":
            void downloadResultImage().catch((error) => showToast(String(error)));
            break;
        case "share-result":
            void shareResult().catch((error) => showToast(String(error)));
            break;
        case "copy-result":
            void copyResult().catch((error) => showToast(String(error)));
            break;
        case "export-log":
            exportLog();
            break;
...
app.addEventListener("input", (event) => {
    ...
    if (input.matches("[data-quadrant-axis]"))
        updateQuadrantWorkbench(input);
});
...
document.addEventListener("keydown", (event) => {
    ...
    if (document.querySelector("dialog[open]"))
        return;
    if (pageMode !== "session" || session?.phase !== "scene")
        return;
    const index = Number(event.key) - 1;
    if (!Number.isInteger(index) || index < 0 || index > 8)
        return;
    const scene = resolveScene(session.state);
    if (RESPONSE_CONFIGS[scene.slotId])
        return;
