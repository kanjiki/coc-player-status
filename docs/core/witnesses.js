...
export const ENDING_WITNESS_PLANS = {
...
export function runWitnessPlan(expectedEnding, plan) {
    ...
    for (let index = 0; index < 24; index += 1) {
        ...
        if (!step)
            throw new Error(`Witness ${expectedEnding} has no step for ${slotId}`);
...
export function runAllEndingWitnesses() {
    return Object.entries(ENDING_WITNESS_PLANS).map(([ending, plan]) => runWitnessPlan(ending, plan));
}
