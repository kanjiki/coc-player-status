...
import { FIXATION_CLUES, MEASUREMENT_ORDER, NETWORK_CLUES, SEAL_CLUES, STOP_CLUES } from "./constants.js";
...
function incrementValueAtPath(root, path, delta) {
...
function modifyCollection(root, path, value, addValue) {
    const current = valueAtPath(root, path);
...
export function conditionMatches(state, condition) {
...
function allConditionsMatch(state, conditions) {
...
export function resolveScene(state, slotId = state.story.currentSlot) {
...
function deterministicD100(state, slotId, choiceId) {
...
function kuramochiProbeScores(state) {
...
function selectKuramochiVariant(state, strategy, diceRoll) {
...
function addKuramochiInformation(state, variant) {
...
function applyEffect(state, effect, diceRoll) {
    switch (effect.op) {
        case "inc":
...
function countKnown(state, registry) {
    return state.story.clues.filter((clue) => registry.has(clue)).length;
}
function recalculateFixationStability(state) {
    ...
    if (state.story.flags.includes("shared_memory_cost_accepted"))
        stability += 1;
    if (state.story.flags.includes("fixation_policy_preserve_relation") && stability >= 2)
        stability += 1;
    if (state.story.flags.includes("body_continuity_fixation") && variant === "B")
        stability += 1;
...
function applyDeadlineIfNeeded(state) {
...
function postSceneUpdates(state, slotId) {
    switch (slotId) {
        case "M03":
...
function recordSalientUnchosen(state, scene, selectedChoiceId) {
    ...
    const key = `act${scene.recordUnchosenAct}`;
    state.story.salientUnchosenActions[key] = candidate.label;
}
function nextMeasurementSlot(slotId) {
...
export function applyChoice(inputState, choiceId, followUpOptionId, diceRollOverride, responseMetadata) {
...
export function fixationClueCount(state) {
    return countKnown(state, FIXATION_CLUES);
}
export function stopClueCount(state) {
    return countKnown(state, STOP_CLUES);
}
export function sealClueCount(state) {
    return countKnown(state, SEAL_CLUES);
}
export function networkClueCount(state) {
 ...
 */
export function applyChoiceForExploration(inputState, choiceId, followUpOptionId, diceRollOverride) {
    const state = cloneState(inputState);
    const slotId = state.story.currentSlot;
    const base = SCENE_BY_SLOT[slotId];
