...
function abstractStateKey(state, step) {
    const keyFlags = [
    ...
    const conditionalClues = [];
    // Keep only clues that can still alter a future Scene variant.
    if (step < 5 && state.story.clues.includes("old_floor_plan"))
        conditionalClues.push("old_floor_plan");
    if (step < 9 && state.story.clues.includes("normalized_clock_in_curve"))
        conditionalClues.push("normalized_clock_in_curve");
    if (step < 16 && state.story.clues.includes("hidden_room_measurement"))
        conditionalClues.push("hidden_room_measurement");
    if (step < 17 && state.story.clues.includes("fuyushiro_shutdown_notes"))
        conditionalClues.push("fuyushiro_shutdown_notes");
    const routes = {};
    if (step < 3)
        routes.lead = state.story.routes.lead;
    if (step < 12)
        routes.echo = state.story.routes.echo;
    if (step < 21)
        routes.finalScope = state.story.routes.finalScope;
    const inventoryConditions = [];
    if (step < 15 && state.story.inventory.includes("metal_tag"))
        inventoryConditions.push("metal_tag");
    if (step < 16 && state.story.inventory.includes("curved_panel_fragment"))
        inventoryConditions.push("curved_panel_fragment");
...
function diceRepresentatives(choice) {
    if (!choice.usesDice)
        return [undefined];
    if (choice.id === "L04_random")
        return [1, 34, 67];
    if ((choice.diceThreshold ?? 100) >= 100)
        return [1];
    return [1, 100];
}
function followUpIds(choice) {
    return choice.followUp ? choice.followUp.options.map((option) => option.id) : [undefined];
}
export function exploreAllAbstractPaths() {
    ...
    const exercisedChoices = new Set();
    const deadEnds = [];
    const invariantViolations = new Set();
    const endingCounts = Object.fromEntries(Object.keys(ENDINGS).map((id) => [id, 0n]));
    let totalAbstractVisited = 1;
    let terminalAbstractStates = 0;
    for (let step = 0; step < MEASUREMENT_ORDER.length; step += 1) {
        if (globalThis.process?.env?.DEBUG_EXPLORE === "1") {
    ...
    return {
        ...
        reachedSlots: [...reachedSlots],
        reachedSceneVariants: [...reachedVariants].sort(),
        exercisedChoiceIds: [...exercisedChoices].sort(),
        endings: Object.fromEntries(Object.entries(endingCounts).map(([id, count]) => [id, count.toString()])),
        deadEnds,
        invariantViolations: [...invariantViolations]
    };
}
export function renderExplorationReport(report) {
