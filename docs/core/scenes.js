const inc = (path, value) => ({ op: "inc", path, value });
const set = (path, value) => ({ op: "set", path, value });
const add = (path, value) => ({ op: "add", path, value });
const reveal = (variant) => ({ op: "revealKuramochi", variant });
const contact = (variant) => ({ op: "contactKuramochi", variant });
const fix = (variant) => ({ op: "fixKuramochi", variant });
const selectInfo = (strategy) => ({
    op: "selectKuramochiInfo",
    strategy
});
const weights = (value) => value;
const condition = (value) => value;
const choice = (value) => value;
const fixationFollowUp = {
...
export const SCENE_BY_SLOT = Object.fromEntries(SCENES.map((scene) => [scene.slotId, scene]));
