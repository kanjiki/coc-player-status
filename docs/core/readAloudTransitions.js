 ...
 */
export function getReadAloudTransition(state) {
    if (state.history.length === 0)
        return "";
    const current = state.story.currentSlot;
    ...
    switch (current) {
        ...
        case "M06": {
            if (choice === "M01_ask_sakaki")
                return "榊から言葉の意味を聞き終えると、彼女は机の引き出しから雨声荘に関する資料をいくつか取り出した。";
            if (choice === "M01_analyze_phase")
                return "解析結果を保存する。十七分のずれを画面に残したまま、榊が雨声荘に関する資料を机へ並べた。";
            if (choice === "M01_joint_review")
                return "二人で分離音声を聞き終える。榊は一度だけスピーカーを見返してから、雨声荘に関する資料を机へ並べた。";
            if (choice === "M01_preserve_only")
                return "音声を未加工のまま保存し、現地で照合することにする。榊が出発前に確認できる資料を机へ並べた。";
            return "";
        }
        case "M02": {
        ...
        case "M04": {
            if (choice === "M11_send_out")
                return "同行者を曲面空間へ退避させる。以後の連絡は榊との通信だけに切り替え、あなたは一人で地下へ続く廊下を進む。";
            if (choice === "M11_assign_role")
                return "役割を確認した同行者が再び歩き出す。二人で地下へ続く廊下へ踏み込む。";
            if (choice === "M11_stop_and_support" || choice === "M11_brief_support")
                return "相手が再び移動できることを確認し、止めていた足を地下側へ向ける。";
            return "";
        }
        case "L03": {
            const byChoice = {
        ...
        case "M07": {
            if (hasFlag(state, "shortcut_fall"))
                return "崩落した近道を抜ける途中で足場が崩れた。負傷した同行者を立たせ、瓦礫の向こう側へ出る。そこから先は、古い地下通路が一本続いている。";
        ...
        case "S01": {
            if (choice === "M10_use_route")
                return "別の自分が示した経路だけを利用し、正体についての問いは残したまま観測室手前へ進む。";
            if (choice === "M10_test")
                return "問いへの反応を記録し終えると、別の自分の像は薄れていく。得られた規則を持って観測室手前へ進む。";
            if (choice === "M10_accept_dialogue" || choice === "M10_mixed")
                return "別の自分との会話を終え、示された方向へ進む。角を越えるたび、背後の気配がひとつ増えたように音だけが重なる。";
            return "";
        }
        case "M08": {
            if (state.mythos.houndStage >= 2)
                return "観測窓から離れても、角の奥にいた細長い影の気配は消えない。窓を背にし、近くの保管棚へ視線を移す。";
            if (choice === "S01_isolate")
                return "曲面材で補助窓を覆う。向こう側の像が隠れたところで、観測室前の保管棚へ向かう。";
            if (choice === "S01_destroy_record")
                return "破損した記録窓から離れる。砕けた縁に残った角を避け、観測室前の保管棚へ向かう。";
            return "観測窓で得た情報を残し、すぐ近くの保管棚へ移る。";
        }
        case "M12": {
            if (state.observer.angleNetworkState === "mapped")
                return "四棟を結ぶ線と金属部材の向きを記録し、倉持の捜索へ戻る。資料を閉じた直後、前方の壁に見覚えのない輪郭が浮かび始めた。";
        ...
        case "M15": {
            if (hasFlag(state, "floor_shifted_underfoot") || hasFlag(state, "marker_route_shifted"))
                return "変形する床をどうにか渡りきる。背後で床板が再びずれ、戻り道の形を変えたところで、三方を壁に囲まれた区画へ出る。";
        ...
        case "S04": {
            if (state.mythos.houndStage >= 2)
                return "観測窓から離れても、角の奥に見えた影は完全には消えない。装置のそばで音が一度鳴るたび、別の角へ細い影が移る。その状態のまま、倉持の記録装置を起動する。";
            if (choice === "M14_isolate_section")
                return "観測窓を曲面材で覆い、漏出をいったん弱める。確保した時間を使い、倉持の記録装置へ向かう。";
            return "観測窓への対応を終え、室内に残された倉持の記録装置へ向かう。";
        }
        case "M13": {
            if (choice === "S04_full_record")
                return "三つの記録を最後まで再生し終えた瞬間、装置の表示が一斉に乱れる。";
            if (choice === "S04_fragments")
                return "断片の照合を終え、A・B・Cの分岐を整理したところで、建物全体に振動が走る。";
            if (choice === "S04_rescue_only")
                return "救出条件だけを抜き出し、再生装置を閉じる。その直後、雨声荘全体が大きく軋んだ。";
            if (choice === "S04_discard_record")
                return "記録を閉じる操作を終えた直後、雨声荘全体が大きく軋んだ。";
            return "";
        }
        case "M16": {
            if (choice === "M13_evacuate")
                return "人物と証拠を球殻型退避槽へ移し終える。榊との通信をつないだまま観測室へ戻ると、中央の像と壁内の振動はまだ続いている。";
            if (choice === "M13_warn_network")
                return "他三施設へ停止指示を送り終える。返信を待ちながら観測室へ戻ると、壁内の振動が四方へ伸びているのが分かる。";
            if (choice === "M13_local_now")
                return "露出した角への応急封鎖を終え、観測室へ戻る。局所の煙は減ったが、壁内の振動は止まっていない。";
            if (choice === "M13_auto_warning")
                return "最低限の封鎖と警告を並行して済ませ、観測室へ戻る。どの処理も途中のまま、中央の装置は動き続けている。";
            return "";
        }
        case "L04": {
        ...
        case "S03": {
            ...
            if (source === "A")
                return "最初に詳しく話を聞いたAの声が、他の二人より先にはっきり聞こえる。だが制御盤には、三つの状態すべてが同じ倉持から分かれたものとして表示されている。";
            if (source === "B")
                return "最初に詳しく確認したBは、三人の中で最も輪郭が安定している。だが制御盤には、三つの状態すべてが同じ倉持から分かれたものとして表示されている。";
            if (source === "C")
                return "最初に詳しく話を聞いたCは、榊の端末へ何度も視線を向けている。だが制御盤には、三つの状態すべてが同じ倉持から分かれたものとして表示されている。";
            return "";
        }
        case "S02": {
...
//# sourceMappingURL=readAloudTransitions.js.map